...
void partiziona_it(nodo_x * N, int k){
    while(k < N->fix->n_ele){
        int count = 0;
        nodo * aux = N->fix->primo;
        while(count < k-1){
            aux = aux->next;
            count++;
        }
        FIFOX * nuova = new FIFOX(aux->next, N->fix->ultimo, aux->next->info, N->fix->ultimo->info, N->fix->n_ele - k);
        //dopo aver creato la nuova fifox, sistemo la vecchia
        aux->next = 0;
        N->fix->ultimo = aux;
        N->fix->val_u = aux->info;
        N->fix->n_ele = k;
        //dopo aver sistemato la vecchia fifox creo un nuovo nodo_x a cui assegno la nuova FIFOX
