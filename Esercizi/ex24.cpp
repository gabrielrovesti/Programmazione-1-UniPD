...
Partendo dal presupposto che l'albero è ben formato (PRE),
si controlla se l'albero sia vuoto o meno,
restituendo l'altezza ritrovata fino a quel momento.
Se il nodo esiste, abbiamo due casi;
-essendo un ordine prefisso, viene prima stampata la radice,
poi il nodo sinistro e, dopo la ricorsione di x nodi, quello destro.
La PRE_RIC in entrambi i casi considera l'albero ben formato e non vuoto,
avendo già passato quel caso base ovviamente.
La POST_RIC darà per scontato lo spostamento verso il sottoalbero
destro/sinistro, a seconda dello spostamento definito nella chiamata ricorsiva.
Induttivamente rimane giusto fino al caso n, vale a dire quando l'albero non ha
più nodi e si ferma la ricorsione.

Nel caso in cui il nodo non esista,
la situazione è similare, ma si ferma la ricorsione dopo x nodi a sinistra e 
ritornando l'ultimo nodo destro, come definisce POST.
