...
nodoE* PM(nodo*T, int*&P, int &dimP){
    ...
    if(T->info == *P){
        ...
        nodoE *pat_albero = conc(x,y);      //concateno i due nodoE (vengono pensate come liste e quindi, essendo conc una funzione fornita, si attaccano i 2 pezzi) e si ritorna
        return new nodoE(T, pat_albero);    //essendo pensati come due liste, nodoE sarebbe un po' come doppiaL e restituisce la parte L della lista e l'albero matchato
    }
...
//Correttezza:
/*
Si consideri la funzione, che ha come parametri l'albero binario T, un array di interi P ben definito
e la sua dimensione dimP. 
Se l'albero T non esiste e siamo al di fuori della dimensione di P, termino il programma
restituendo coerentemente con POST il resto dell'albero trovato.
Qualora T->info dovesse essere uguale a *P abbiamo due casi in particolare:
-se uguali, incremento P e diminiusco dimP, essendo passati per riferimento
e ricorriamo a sinistra, destra finché ci sono ricorsivamente da entrambe le parti
(sapendo che da PRE l'albero è ben formato, sarà vera anche PRE_RIC
e POST_RIC, che garantiscono la restituzione di un sottoalbero dx/sx finché
ci sono elementi).
A questo punto viene coerentemente con POST restituito l'albero matchato (concatenando x ed y)
e l'albero originale.
-se invece la condizione fosse falsa, semplicemente viene restituita una lista di nodi
che puntano all'albero originale, dimostrando la veridicità di POST.
*/
