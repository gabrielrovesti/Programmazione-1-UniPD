...
//PRE_inizio_cam=(C contiene n cammini (sequenze possibilmente vuote di 0 e 1),
// ogni cammino termina con -1 e dopo l'ultimo cammino (che termina, come tutti i cammini, con -1) c'è la sentinella -2, n >= 0, i >= 0)
...
}//POST_inizio_cam=(se i in [0..n-1] allora inizio_cam restituisce l'indice di C 
//in cui inizia il cammino i, altrimenti, inizio_cam restituisce -2)
...
/*PROVA DI CORRETTEZZA: PRE_inizio_cam <inizio_cam()> POST_inizio_cam
	...
	Caso base: *C==-2
-------------------
Dato PRE_inizio_cam, se *C==-2 allora abbiamo già trovato gli n cammini (non ne dobbiamo cercare altri) e
	restituiamo con il return -2 → vale POST_inizio_cam
...
----------------------------------------------------------
Dato PRE_inizio_cam, invoco la chiamata ricorsiva inizio_cam(C+1, i), con C+1 definito in quanto *C!=-2 
	(non stavamo considerando l'ultima posizione) e i>=0: se la funzione ritorna -2 allora abbiamo trovato 
	tutti gli n cammini e ritorniamo -2 → valgono PRE_inizio_cam e POST_inizio_cam

	Caso ricorsivo: 1+inizio_cam(C+1, i) && *C!=-2 && i!=0 && inizio_cam(C+1, i)!=-2
----------------------------------------------------------------------------------
Dato PRE_inizio_cam, se non ho già considerato tutti gli n cammini, invoco la chiamata ricorsiva inizio_cam(C+1, i),
	con C+1 definito in quanto *C!=-1, *(C+1)!=-2 e i>=0: la funzione retituirà indice dell'inizio del 
	cammino -1 partendo da C+1 al quale verrà sommato 1 per avere l'indice dell'inizio del cammino 
	partendo da C → valogno PRE_inizio_cam e POST_inizio_cam
...
//PRE_add_ric=(C[0...] contiene una sequenza w di 0 e 1 (possibilmente vuota) seguita da -1, r è un albero corretto,
// INP contiene almeno un intero che chiamiamo x, r=vr)
